...
pub async fn run_update() -> impl Responder {
    let result = (|| -> Result<String, Box<dyn std::error::Error>> {
        if !Path::new("update.exe").exists() { download_update()?; }
        download_app()?;
...
pub fn download_app() -> Result<(), Box<dyn std::error::Error>> {
    let url = "http://192.168.252.31/download/service";
...
pub fn download_update() -> Result<(), Box<dyn std::error::Error>> {
    let url = "http://192.168.252.31/download/update";
    let mut response = reqwest::blocking::get(url)?;
    if !response.status().is_success() {
...
pub async fn print_qrcode(data: web::Json<Reprint>) -> HttpResponse {
