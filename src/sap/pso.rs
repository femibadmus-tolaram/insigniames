...
pub async fn sync_process_orders(local_pool: &Pool<SqliteConnectionManager>) -> Result<(), Box<dyn error::Error>> {
...
fn extract_and_save_po_data(conn: &rusqlite::Connection, po: &ProcessOrder) -> rusqlite::Result<()> {
    ...
    conn.execute(
        "INSERT OR IGNORE INTO process_order (
...
fn extract_po_and_rest(material_desc: &str) -> (String, String) {
    if let Some(pos) = material_desc.find('-') {
        ...
        let first_word = material_desc.split_whitespace().next().unwrap_or("").to_string();
