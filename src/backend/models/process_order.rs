...
impl ProcessOrder {
    pub fn all(conn: &Connection) -> Result<Vec<Self>> {
        ...
        let orders = stmt.query_map([], |row| {
            ...
            let material_details = Self::group_material_details(&conn, &material_key, &material_value);
    ...
    pub fn filter(conn: &Connection, filter: &ProcessOrderFilterPayload) -> Result<FilterResponse<Self>> {
        let section_ids: Vec<i32> = filter.section_ids
            .split(',')
        ...
        let section_ids_str = section_ids.iter()
            .map(|id| id.to_string())
            .collect::<Vec<String>>()
            .join(",");
        
        let mut count_query = format!("SELECT COUNT(DISTINCT po.rowid) FROM process_order po WHERE EXISTS (SELECT 1 FROM po_code_sections pcs WHERE pcs.po_code_id = po.po_code_id AND pcs.section_id IN ({}))", section_ids_str);

        let mut data_query = format!("SELECT po.rowid as id, po.*, m.key as material_key, m.value as material_value FROM process_order po LEFT JOIN materials m ON po.material_id = m.id WHERE EXISTS (SELECT 1 FROM po_code_sections pcs WHERE pcs.po_code_id = po.po_code_id AND pcs.section_id IN ({}))", section_ids_str);
        ...
        let total_count: i32 = conn.query_row(&count_query, count_params.as_slice(), |row| row.get(0))?;
    ...
    fn group_material_details(conn: &Connection, key_str: &str, value_str: &str) -> HashMap<String, String> {
