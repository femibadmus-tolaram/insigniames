...
impl Machine {
    pub fn create(conn: &Connection, data: &MachineCreatePayload) -> Result<Self> {
        conn.execute(
            "INSERT INTO machines (name, label) VALUES (?1, ?2)",
            params![data.name, data.label],
        )?;
    ...
    pub fn delete(&self, conn: &Connection) -> Result<()> {
        conn.execute("DELETE FROM user_machines WHERE machine_id = ?1", params![self.id])?;
    ...
    pub fn get_stats(conn: &Connection) -> Result<MachineStats> {
        ...
        let total_users: i32 = conn.query_row("SELECT COUNT(DISTINCT user_id) FROM user_machines", [], |row| row.get(0))?;
    ...
    pub fn filter(conn: &Connection, filter: &MachineFilterPayload) -> Result<FilterResponse<Self>> {
        let mut count_query = "SELECT COUNT(DISTINCT m.id) FROM machines m WHERE 1=1".to_string();
        let mut data_query = "SELECT m.*, 
                             COUNT(DISTINCT um.user_id) as user_count,
                             COUNT(DISTINCT j.id) as job_count
                             FROM machines m
                             LEFT JOIN user_machines um ON m.id = um.machine_id
        ...
        if let Some(val) = &filter.has_users {
            if val == "true" {
                count_query.push_str(" AND EXISTS (SELECT 1 FROM user_machines um WHERE um.machine_id = m.id)");
                data_query.push_str(" AND EXISTS (SELECT 1 FROM user_machines um WHERE um.machine_id = m.id)");
            } else if val == "false" {
                count_query.push_str(" AND NOT EXISTS (SELECT 1 FROM user_machines um WHERE um.machine_id = m.id)");
                data_query.push_str(" AND NOT EXISTS (SELECT 1 FROM user_machines um WHERE um.machine_id = m.id)");
            }
        }
        ...
        data_query.push_str(" GROUP BY m.id ORDER BY m.name");
