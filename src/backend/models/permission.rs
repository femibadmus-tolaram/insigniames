...
impl Permission {
    pub fn create(conn: &Connection, p: &PermissionPayload) -> Result<Self> {
        conn.execute(
            "INSERT INTO permissions (content_type_id, can_create, can_read, can_update, can_delete) 
             VALUES (?1, ?2, ?3, ?4, ?5)",
            params![p.content_type_id, p.can_create, p.can_read, p.can_update, p.can_delete],
        )?;
        ...
        let mut stmt = conn.prepare(
            "SELECT c.model FROM content_type c
             JOIN permissions p ON p.content_type_id = c.id
             WHERE p.id = ?1",
        )?;
        let model: String = stmt.query_row([id], |row| row.get(0))?;
    ...
    pub fn update(&mut self, conn: &Connection, p: &PermissionUpdatePayload) -> Result<()> {
        if let Some(can_create) = p.can_create {
            conn.execute("UPDATE permissions SET can_create = ?1 WHERE id = ?2",
                         params![can_create, self.id])?;
            self.can_create = can_create;
        }
        if let Some(can_read) = p.can_read {
            conn.execute("UPDATE permissions SET can_read = ?1 WHERE id = ?2",
                         params![can_read, self.id])?;
            self.can_read = can_read;
        }
        if let Some(can_update) = p.can_update {
            conn.execute("UPDATE permissions SET can_update = ?1 WHERE id = ?2",
                         params![can_update, self.id])?;
            self.can_update = can_update;
        }
        if let Some(can_delete) = p.can_delete {
            conn.execute("UPDATE permissions SET can_delete = ?1 WHERE id = ?2",
    ...
    pub fn delete(conn: &Connection, id: i32) -> Result<()> {
        conn.execute("DELETE FROM role_permissions WHERE permission_id = ?1", params![id])?;
    ...
    pub fn count_linked_roles(conn: &Connection, permission_id: i32) -> Result<i32> {
        let mut stmt = conn.prepare("SELECT COUNT(*) FROM role_permissions WHERE permission_id = ?1")?;
    ...
    pub fn all_for_user(conn: &Connection, user_id: i32) -> Result<Vec<Self>> {
        ...
        let rows = stmt.query_map([user_id], |row| Ok(Permission {
            id: row.get(0)?,
            role_id: row.get(1)?,
            model: row.get(2)?,
    ...
    pub fn all(conn: &Connection) -> Result<Vec<Self>> {
        ...
        let rows = stmt.query_map([], |row| Ok(Permission {
            id: row.get(0)?,
            role_id: row.get(1)?,
            model: row.get(2)?,
            can_create: row.get(3)?,
