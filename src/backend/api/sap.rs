...
pub async fn process_order(search: web::Query<std::collections::HashMap<String, String>>) -> impl Responder {
    let user = env::var("SAP_QA_USERNAME").unwrap_or_default();
    let pass = env::var("SAP_QA_PASSWORD").unwrap_or_default();
    let base_url = env::var("SAP_QA_URL").unwrap_or_default();
    let mut url = format!("{}?$format=json&$top=10", base_url);

    let mut filters = Vec::new();
    filters.push("Plant eq 'A710'".to_string());
    // filters.push("ManufacturingOrder eq '220006680'".to_string());
    filters.push("ManufacturingOrderType eq 'ZIS1'".to_string());
